...
async exportTrabajadoresFisicosExcel(
  ...
  try {
    ...
    res.set({
      'Content-Type':
        'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet',
      'Content-Disposition': `attachment; filename=trabajadores_fisicos_${fecha}.xlsx`,
    });
...
async exportTrabajadoresFisicosPdf(
  ...
  try {
    const buffer = await this.exportService.generateTrabajadoresFisicosPdf(fecha);

    res.set({
      'Content-Type': 'application/pdf',
      'Content-Disposition': `attachment; filename=trabajadores_fisicos_${fecha}.pdf`,
    });
