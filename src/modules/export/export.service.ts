...
export class ExportService {
  ...
  // Listar trabajadores interruptos dadas su UEB y fecha (mes/año)
  public async listInterruptosUEB(ueb: string, fecha: string) {
    try {
      const client = axios.create({ baseURL: this.baseUri });

      const trabajadores = await client.get(
        `/recursosHumanos/interruptosUEB?ueb=${ueb}&fecha=${fecha}`,
      );
  ...
  // Listar cantidad de trabajadores por claves de ausentismo 
  // validar si los datos obtenidos del servicio son correctos o estan completos 
  // la funcion debe obtener la cantidad de trabajadores que existe por clave de ausentismo
  ...
  public async listAusentismoClaves(ueb: string, fecha: string) {
    ...
    try {
      ...
      //tengo dudas con este error con el logger
